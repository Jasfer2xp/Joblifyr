...
export default function JoblifyrApp() {
  ...
  useEffect(() => {
    ...
    if (location.state?.verifyEmail) {
      setPendingVerifyEmail(location.state.verifyEmail);
    }
      ...
      <div className="flex-grow flex flex-col">
        ...
        {currentView === 'register' && (
          <RegisterView
            ...
            onRegistered={setPendingVerifyEmail}
          />
        )}
        {currentView === 'verify' && (
          <VerifyView
            navigateTo={navigateTo}
            email={pendingVerifyEmail || regEmail || ''}
          />
        )}
        {currentView === 'profile' && (
          <ProfileView
...
/* =================================================================== */
function LoginView({ navigateTo, email, setEmail, password, setPassword, showPassword, setShowPassword, authError }) {
  const { login } = useAuth();
        ...
        <div className="w-full max-w-md bg-white rounded-3xl p-8 sm:p-10 border border-slate-200/80 shadow-xl shadow-slate-200/40">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full bg-[#4F52E6] hover:bg-[#4345D9] disabled:opacity-60 text-white font-semibold py-3.5 rounded-xl shadow-lg shadow-indigo-500/20 transition text-sm text-center"
            >
          ...
          <div className="space-y-3">
            ...
            >
              <ArrowRight size={16} className="text-indigo-600" /> Continue with Google
            </button>
            <button
              type="button"
              onClick={() => navigateTo('jobs')}
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await register({
        ...
        first_name,
        last_name,
        role: role === 'hire' ? 'employer' : 'job_seeker',
      });
      onRegistered?.(email);
      navigateTo('verify', { verifyEmail: email });
    } catch (err) {
          ...
          <div className="lg:col-span-6 bg-white rounded-3xl p-8 sm:p-10 border border-slate-200/80 shadow-xl max-w-lg mx-auto lg:max-w-none w-full">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full bg-[#4F52E6] hover:bg-[#4345D9] disabled:opacity-60 text-white font-semibold py-3.5 rounded-xl shadow-lg shadow-indigo-500/20 transition text-sm flex items-center justify-center gap-2 mt-4"
              >
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <button
                type="button"
                onClick={() => alert('LinkedIn OAuth coming soon. Please use Google or email registration.')}
                className="w-full bg-white hover:bg-slate-50 border border-slate-200 text-slate-700 font-semibold py-2.5 rounded-xl transition flex items-center justify-center gap-2 text-xs"
              >
